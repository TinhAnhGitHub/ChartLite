...
def save_checkpoint(config: dict, state: dict, checkpoint_name) -> None:
    """
    ...
    Notes:
        ...
        - Optionally copies the best model checkpoint
    """
    os.makedirs(config["outputs"]["model_dir"], exist_ok=True)
    filename = f'{config["outputs"]["model_dir"]}/{checkpoint_name}'
...
def setup(rank: int, world_size: int, timeout_seconds: float = 30.0):
    """
